...
public class TypeCheckVisitor extends WACCParserBaseVisitor<Tree> {
	...
	/* ident PLUSEQUALS | MINUSEQUALS expr */
	@Override
	public Tree visitConditionalOps(@NotNull WACCParser.ConditionalOpsContext ctx) {
		BinaryOperators op = BinaryOperators.PLUSEQUAL;
		if(ctx.condAssign().MINUSEQUAL() != null) {
			op = BinaryOperators.MINUSEQUAL;
		}
		ExprNode variable = (ExprNode) visit(ctx.condAssign().ident());
		ExprNode liter = (ExprNode) visit(ctx.condAssign().expr());
		ex_CompoundAssignNode assign = new ex_CompoundAssignNode(variable, op, liter);
		return assign;
	}
	...
	/* ident ADD expr */
	@Override
	public Tree visitAddElemList(@NotNull WACCParser.AddElemListContext ctx) {
		Variable listNode =  (Variable) currentSymbolTable.get(ctx.ident().getText());
		ExprNode exprNode = (ExprNode) visit(ctx.expr());
		ex_AddElemListNode addElemListNode = new ex_AddElemListNode(listNode.getListObject(), exprNode);
		addElemListNode.check(currentSymbolTable, ctx);
		return addElemListNode;
	}

	/* ident ADD expr expr */
	@Override public Tree visitAddElemMap(@NotNull WACCParser.AddElemMapContext ctx) {
		Variable mapNode =  (Variable) currentSymbolTable.get(ctx.ident().getText());
		ExprNode key = (ExprNode) visit(ctx.expr(0));
		ExprNode value = (ExprNode) visit(ctx.expr(1));
		ex_AddElemMapNode addElemMapNode = new ex_AddElemMapNode(mapNode.getMapObject(), key, value);
		addElemMapNode.check(currentSymbolTable, ctx);
		return addElemMapNode;
	}
...
//..................................ASSIGNMENT......................................
	...
	public Tree visitNewList_assignRHS(NewList_assignRHSContext ctx) {
		...
		ex_listObject listObject = new ex_listObject();
		ex_NewList list = new ex_NewList(type, listObject);
	...
	public Tree visitNewMap_assignRHS(NewMap_assignRHSContext ctx) {
		...
		ex_mapObject mapObject = new ex_mapObject();
		ex_NewMap map = new ex_NewMap(type1, type2, mapObject);
	...
	/* ident GETLIST expr */
	@Override
	public Tree visitListElem(@NotNull WACCParser.ListElemContext ctx) {
		String ident = ctx.ident().getText();
		ExprNode expr = (ExprNode) visit(ctx.expr());
		Variable var = (Variable) currentSymbolTable.get(ident);
		ex_ListElem listElem = new ex_ListElem(expr, var);
		listElem.check(currentSymbolTable, ctx);
		return listElem;
	}

	/* ident GETMAP expr */
	@Override
	public Tree visitMapElem(@NotNull WACCParser.MapElemContext ctx) {
		String ident = ctx.ident().getText();
		ExprNode expr = (ExprNode) visit(ctx.expr());
		Variable var = (Variable) currentSymbolTable.get(ident);
		ex_MapElem mapElem = new ex_MapElem(expr, var);
		mapElem.check(currentSymbolTable, ctx);
		return mapElem;
	}
	...
	public Tree visitExpr(ExprContext ctx) {
		...
		// Ternary expression: expr ? expr : expr
		if(ctx.TERNARY() != null) {
			ExprNode expr = (ExprNode) visit(ctx.expr(0)) ;
			ExprNode ifExpr = (ExprNode) visit(ctx.expr(1));
			ExprNode thenExpr = (ExprNode) visit(ctx.expr(2));
			ex_TernaryExpr ternaryExpr = new ex_TernaryExpr(expr, ifExpr, thenExpr);
			ternaryExpr.check(currentSymbolTable, ctx);
			return ternaryExpr;
		}
		...
		switch (ctx.getChildCount()) {
			case 3:
				...
				if(ctx.ident() != null && (ctx.PLUS(0) != null && ctx.PLUS(1) != null|| ctx.MINUS(0) != null &&
						ctx.MINUS(1) != null)) {
					ExprNode expr = (ExprNode) visit(ctx.ident());
					UnaryOperators unaryOp = UnaryOperators.evalUnOp(ctx.getChild(1).getText() + ctx.getChild(2).getText());
